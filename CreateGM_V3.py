...
class SVGParser:
    ...
    def get_element_bbox(self, element, parent_transform=np.identity(3)):
        ...
        if tag == "rect":
            ...
            bbox = np.array([[x, x + width/2.0, x + width], [y, y + height/2.0,  y + height]])
        ...
        # 处理椭圆
            ...
            bbox = np.array([[cx - rx, cy - ry], [cx + rx, cy + ry]])
    ...
    def write_output(self):
        with open("print.json", "w") as file:
            file.write(
                str(self.file_path).split("/")[1] + " " + str(self.graph) + "\n\n"
            )
            ...
            for element in SVGParser.parse_svg(self.file_path).iter():
...
if __name__ == "__main__":
    svg_parser = SVGParser("./untitled.svg")
    svg_parser.run()
